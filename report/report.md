...
### Introduction
As Docker becomes "the" production tool of choice, it is not a surprise that our final AIT lab is about the leader in application dockerization. In this lab we will dive in the load balancing with a dynamic containers infrastructure. We will build docker images and become experts (or at least familiar) at Docker lightweight process supervision. We will understand all the core concepts for dynamic scaling of an application in production and put the decentralized management of web server instances into practice.
...
1. Take a screen-shot of the stats page of HAProxy at <http://192.168.42.42:1936>. You should see your backend nodes. It should be really similar to the screen-shot of the previous task.
...
**[M4]** To manage our load balancer's auto configuration we introduce the `Serf` tool which will let our containers communicate between themselves in a decentralized cluster. This lets the load balancer configure its reachable host's list more dynamically than before.
...
4. (Optional:) Propose a different approach to manage the list of backend nodes. You do not need to implement it. You can also propose your
   own tools or the ones you discovered on-line. In that case, do not forget to cite your references.

### Task 6

**Deliverables**:

1. Take a screen-shots of the HAProxy stat page showing more than 2 web applications running. Additional screen-shots are welcome to see a
   sequence of experimentations like shutting down a node and starting more nodes.

   Also provide the output of `docker ps` in a log file. At least one file is expected. You can provide one output per step of your experimentation according to your screen-shots
...
We did this lab without too many difficulties, however we can say that working during the holidays was not always easy, and even more when we also had to work on multiple different labs and project.

### Conclusion

And like this the lab comes to an end. We are now familiarized with dynamic container infrastructure, as we practiced the decentralized management of a web server. We understand the lightweight process supervision offered by Docker and are ready for any or mostly any future load balancing Docker usage, which we are sure to see many.